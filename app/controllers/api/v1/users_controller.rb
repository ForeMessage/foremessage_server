class Api::V1::UsersController < ApplicationController
  ...
  def check_in
    ...
    success_response(extra_parameters: { access_token: @auth_secret.create_token(user, 'access_token'), refresh_token: user.token.refresh_token })
  end
  ...
  def sign_in
    ...
    begin
      ...
      success_response(extra_parameters: { access_token: access_token, refresh_token: refresh_token })
    rescue => e
  ...
  def sign_up
    ...
    begin
      ...
      success_response(extra_parameters: { access_token: @auth_secret.create_token(user, 'access_token'), refresh_token: user.token.refresh_token }, status: :created)
    rescue => e
      error_response(status: :bad_request, message: e.message) and return
    end
  end
